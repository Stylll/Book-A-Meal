    ...
    it('should create menu with current date', (done) => {
      request(app)
        ...
        .expect((err, resp) => {
          ...
          expect(resp.body.date).to.equal(getNormalDate(new Date()));
          expect(resp.body.name).to.equal(`Menu For ${beautifyDate(new Date())}`);
          expect(resp.body.createdAt).to.be.a('date');
          expect(resp.body.updatedAt).to.be.a('date');
